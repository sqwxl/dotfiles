...
local keymaps = {
    core = {
        ...
        { "<A-S-Down>", "<esc><cmd>m .+1<cr>==gi",                                   desc = "Move Down",     mode = "i" },
        { "<A-S-Up>",   "<esc><cmd>m .-2<cr>==gi",                                   desc = "Move Up",       mode = "i" },
        { "<A-S-Down>", ":<C-u>execute \"'<,'>move '>+\" . v:count1<cr>gv=gv",       desc = "Move Down",     mode = "v" },
        { "<A-S-Up>",   ":<C-u>execute \"'<,'>move '<-\" . (v:count1 + 1)<cr>gv=gv", desc = "Move Up",       mode = "v" },
        { "<",          "<gv",                                                       mode = "v" },
        { ">",          ">gv",                                                       mode = "v" },
        ...
        { "<A-j>",   "<C-Bslash><C-N><C-w>j",                             desc = "Move down",                         mode = { "t", "i" } },
        { "<A-k>",   "<C-w>k",                                            desc = "Move up" },
        { "<A-k>",   "<C-Bslash><C-N><C-w>k",                             desc = "Move up",                           mode = { "t", "i" } },
        { "<A-l>",   "<C-w>l",                                            desc = "Move right" },
        { "<A-l>",   "<C-Bslash><C-N><C-w>l",                             desc = "Move right",                        mode = { "t", "i" } },
    ...
    leader = {
        ...
        -- location list
        {
            "<Leader>xl",
            function()
                local success, err = pcall(vim.fn.getloclist(0, { winid = 0 }).winid ~= 0 and vim.cmd.lclose or
                    vim.cmd.lopen)
        ...
        -- quickfix list
        {
            "<Leader>xq",
            function()
                local success, err = pcall(vim.fn.getqflist({ winid = 0 }).winid ~= 0 and vim.cmd.cclose or vim.cmd
                    .copen)
                if not success and err then vim.notify(err, vim.log.levels.ERROR) end
            end,
            desc = "Quickfix List"
        ...
        { "<leader><c-w>",   function() require("which-key").show({ keys = "<c-w>", loop = true }) end,                                     desc = "Window Hydra Mode (which-key)" },
        { "<Leader>$",       function() Snacks.picker.recent() end,                                                                         desc = "Recent" },
        { "<Leader>,",       function() Snacks.picker.buffers() end,                                                                        desc = "Buffers" },
        { "<Leader>.",       function() Snacks.picker.smart() end,                                                                          desc = "Smart Find Files" },
        { "<Leader>/",       function() Snacks.picker.grep() end,                                                                           desc = "Grep" },
        { "<Leader>:",       function() Snacks.picker.command_history() end,                                                                desc = "Command History" },
        { "<Leader>A",       function() vim.lsp.buf.code_action({ apply = true, context = { only = { "source" }, diagnostics = {} } }) end, desc = "Source Action" },
        { "<Leader>a",       vim.lsp.buf.code_action,                                                                                       desc = "Code Action",                  mode = { "n", "v" } },
        { "<Leader>d",       vim.diagnostic.open_float,                                                                                     desc = "Line Diagnostics" },
        { "<Leader>e",       "<Cmd>Neotree toggle=true<CR>",                                                                                desc = "Toggle Neotree" },
        ...
        { "<Leader>k",       function() return vim.lsp.buf.hover() end,                                                                     desc = "Hover" },
        {
            ...
            function()
                ...
                if nr ~= nil then vim.cmd.wincmd(nr .. " w") end
            end,
            desc = "Pick window"
        },
        ...
        { "<Leader>gg", function() Snacks.terminal({ "gitui" }) end,                                                  desc = "GitUi (Root Dir)" },
        ...
        { "<Leader>r",  vim.lsp.buf.rename,                          desc = "Rename" },
        {
            ...
            function()
                ...
                vim.ui.input({ prompt = 'Replace "' .. old .. '" with: ', default = old, completion = "buffer" },
                    function(new)
                        if new ~= nil then
